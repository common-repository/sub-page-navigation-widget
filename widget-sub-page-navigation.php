...
Class wp_widget_sub_page_navigation Extends WP_Widget {
  ...
  Function Widget ($widget_args, $options){
    ...
    ElseIf ( Is_Home() && !Is_Front_Page() ){
      $current_page = $GLOBALS['wp_query']->queried_object;
    }
    Else
      return False;
    ...
    // Show widget data
    If ($navigation->have_posts() && $current_page->post_parent == 0){
      $widget_title = Apply_Filters('the_title', $current_page->post_title);
      $parent_id = False;
    }

    ElseIf ($navigation->have_posts() && $current_page->post_parent != 0){
      $widget_title = Apply_Filters('the_title', $current_page->post_title);
    ...
    ElseIf (!$navigation->have_posts() && $current_page->post_parent != 0){
      $widget_title = get_the_title($current_page->post_parent);
      $navigation->Query(Array(
    ...
    $template_file = Get_Query_Template('sub-page-navigation');
    If (!Is_File($template_file)) $template_file = DirName(__FILE__) . '/widget.php';
    
    // run the filter for the template file
    $template_file = Apply_Filters('sub_page_navigation_template', $template_file);
  ...
  Function ExplodeInt($delimiter, $string){
    ...
    ForEach ($arr_parts AS $index => &$value) $value = IntVal(Trim($value));
    return $arr_parts;
  }
